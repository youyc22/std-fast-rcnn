...
import os
import random
os.environ['TORCH_HOME'] = 'assets/ckpts'
...
def main(args):
    ...
    train_dataset = get_pascal_voc2007_data('./data/GDM9ZH/VOCtrainval_06-Nov-2007/', 'train')
    val_dataset = get_pascal_voc2007_data('./data/GDM9ZH/VOCtrainval_06-Nov-2007/', 'val')

    train_loader = pascal_voc2007_loader(train_dataset, args.batch_size, shuffle=True, num_workers=args.num_workers,
                                         proposal_path='./data/GDM9ZH/VOCtrainval_06-Nov-2007/VOCdevkit/VOC2007/Proposals')
...
def train(args, model, train_loader, optimizer, lr_scheduler, start_epoch):
    ...
    for i in range(start_epoch, args.epochs):
        ...
        mean_loss.append(np.mean(loss_history[-len(train_loader):]))
        lr_scheduler.step()
...
def inference(args, model, val_loader, dataset, thresh=0.2, nms_thresh=0.30, visualize=True):
    ...
    for iter_num, data_batch in enumerate(val_loader):
        ...
        for idx in range(batch_size):
            ...
            #过滤掉idx为20（背景）的类别
            resized_final_proposals = resized_final_proposals[resized_final_proposals[:, 4] != 20]
